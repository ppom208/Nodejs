...
const getApparelSizes = catchAsync(async (req, res) => {
  ...
  const result = await apparelSizeService.queryApparelSize(filter, options);
  res.send(result);
});

const getApparelSize = catchAsync(async (req, res) => {
  const apparelSize = await apparelSizeService.getApparelSizeById(req.params.apparelSizetId);
  if (!apparelSize) {
